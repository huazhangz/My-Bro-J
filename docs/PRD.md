...
| `PET_SIZE_HUGE_SHIFT_X` | `3` | 超大档 Steve 再向右偏 3 格 |
| `DRYER_ICON_NUDGE_Y` | `1` | 烘干机图标在按钮内下移 1 格（按钮位置不变） |
| `DRAWER_ICON_NUDGE_Y` | `4` | 抽屉图标在按钮内下移 4 格（按钮位置不变） |
...
| `INVENTORY_SCALE` | `2.5` | 烘干机 / 抽屉弹层相对**未缩放** `PET_AREA` 的放大倍数，不跟体型档 |
...
- **ExitPopup**（根节点下，默认隐藏）：在立绘上 **右键** 弹出 **4 倍**圆角菜单，窗口放大并按屏幕位置夹紧
  ...
  - 顶部气泡：❤好感度 / 「洗了 xx 条 内裤」 / ⏰陪伴时长（HH：MM：SS 墙钟累计） / 🏃跑路次数（高度 +65%）
  - 其下烘干机 / 抽屉：左图标右文字的 Button（高度 +35%）；抽屉图标用 `drawer1.jpg` 抠绿，库存背景仍用 `drawer.jpg`
  - 菜单图标相对按钮框下移：烘干机 `DRYER_ICON_NUDGE_Y=1`、抽屉 `DRAWER_ICON_NUDGE_Y=4`
  - 好感度：品质 log 高权重 + 陪伴 log（满值时间 ×260%）最多 15% − 跑路中等扣分
  - `能不能给我洗快点`：随机扣洗涤时间（1s~12h）；冷却灰显读秒
  - `设置`：固定上层 + Steve 体型（小 70% / 中 / 大 135% / 超大 200%，超大再右移 3 格）；体型只缩放立绘窗口，烘干机/抽屉交互窗尺寸固定
  - `晚点再洗`：存档后退出
  - 右上角 `×`：关闭并还原窗口
  - 跑路时立绘换成按比例内接的 `container.jpg` 空盆；「已跑路...」居中贴在画面顶部，z 低于菜单
  - 点弹窗外 / `ESC` / `×`：关闭菜单
- **InventoryPopup**（全屏，默认隐藏）：背景随种类换 `dryer.jpg` / `drawer.jpg`，外加半透明黑遮罩
  - 窗口临时放大为未缩放立绘区域的 **2.5 倍**（`inventory_window_size()`，不跟 Steve 体型）
  - 左上角标题带 headline 色条（烘干机紫 / 抽屉金，与右键按钮底色一致），高度不超过第一件库存
  - 右上角「关闭」与标题条同高；抽屉时关闭左侧多「收拾一下」
  - 「收拾一下」展开品质 + 磨损前缀勾选，命中任一勾选项即从 `dry_collection` 删除（不减生涯「洗了 xx 条」）
...
- [x] **Day 3**：右键菜单 + 库存网格（常驻 HUD / 图鉴 / 加速按钮已拆除）
  ...
  - [x] `InventoryPopup`：dryer/drawer 绿幕背景 + 5 列滚动网格；标题色条与菜单按钮同色；烘干机背景中心 ×1.36
  - [x] 抽屉「收拾一下」：勾选品质 / 磨损词条后一键删除；菜单统计文案「洗了 xx 条 内裤」
  - [x] 设置体型不联动库存窗；超大 Steve 右移 3 格；菜单图标在按钮内下移
  - [x] 悬停 1.5s 头顶洗涤水条
