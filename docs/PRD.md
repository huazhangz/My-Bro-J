...
| `DRYER_ICON_NUDGE_Y` | `2` | 烘干机图标在按钮内下移 2 格（按钮位置不变） |
| `DRAWER_ICON_NUDGE_Y` | `3` | 抽屉图标在按钮内下移 3 格（按钮位置不变） |
...
| `INVENTORY_SCALE` | `2.5` | 烘干机 / 抽屉弹层相对**未缩放** `PET_AREA` 的基准倍数 |
| `INVENTORY_SIZE_FACTOR` | `0.75` | 库存窗相对上一版再缩到 75%，并裁成宽:高 4:3 |
...
- **ExitPopup**（根节点下，默认隐藏）：在立绘上 **右键** 弹出 **4 倍**圆角菜单，窗口放大并按屏幕位置夹紧
  ...
  - 菜单图标相对按钮框下移：烘干机 `DRYER_ICON_NUDGE_Y=2`、抽屉 `DRAWER_ICON_NUDGE_Y=3`
...
- **InventoryPopup**（全屏，默认隐藏）：背景随种类换 `dryer.jpg` / `drawer.jpg`，外加半透明黑遮罩
  - 窗口为上一版的 **75%**，并裁成宽:高 **4:3**（`inventory_window_size()`，不跟 Steve 体型）
  - 左上角标题带 headline 色条（烘干机紫 / 抽屉金，与右键按钮底色一致），高度不超过第一件库存
  - 右上角「关闭」与标题条同高；抽屉时关闭左侧多「收拾一下」
  - 「收拾一下」：品质集合与磨损集合取交集后删除；勾选框变红；不减生涯「🩲 洗了 xx 条」
...
- [x] **Day 3**：右键菜单 + 库存网格（常驻 HUD / 图鉴 / 加速按钮已拆除）
  ...
  - [x] 抽屉「收拾一下」：品质×磨损交集删除；勾选变红；菜单「🩲 洗了 xx 条」（删除不减计数）
  - [x] 库存窗 75% 且 4:3；超大 Steve 右移 6 格；菜单图标下移 2/3；看电影/约个饭/聊聊天 demo 按钮
  - [x] 悬停 1.5s 头顶洗涤水条
